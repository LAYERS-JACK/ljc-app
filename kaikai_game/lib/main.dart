...
class StageDef {
  ...
  final String story;
  const StageDef({required this.name, required this.waves, required this.types, required this.story});
}
const List<StageDef> stages = [
  StageDef(name:'夜の家', waves:8, types:['normal','red'], story:'押し入れの奥から、ずっと誰かの気配がしていた。'),
  StageDef(name:'廊下の鏡', waves:10, types:['normal','red','fast'], story:'鏡の中で、自分が笑っていた。\n笑った覚えは、ない。'),
  StageDef(name:'とおくのひと', waves:12, types:['normal','red','fast','tough'], story:'遠くに人がいる。\n近づいても、近づいても、遠い。'),
  StageDef(name:'言伝', waves:15, types:['normal','red','fast','tough'], story:'この文章を読んでいる　あなた　に\n伝えたいことが、ある。'),
];
class EnemyType {
  final String emoji;
...
class Enemy {
  ...
  DateTime spawnTime;
  int currentHp;
  Enemy({required this.id, required this.typeKey, required this.position, required this.spawnTime, required this.currentHp});
...
class _GameScreenState extends State<GameScreen> {
  ...
  String _pickType() {
    final avail = _currentStage.types;
  ...
  void _scheduleSpawn() {
    final interval = (_spawnIntervalMs + _rng.nextInt(400) - 200).clamp(600, 2500);
  ...
  void _spawnEnemy() {
    ...
    final key = _pickType();
    setState(() {
      _enemies.add(Enemy(id: DateTime.now().microsecondsSinceEpoch.toString(), typeKey: key, position: Offset(x, y), spawnTime: DateTime.now(), currentHp: enemyTypes[key]!.hp));
    });
  }
  void _checkExpired() {
    if (_stageClear) return;
    final now = DateTime.now();
    final expired = _enemies.where((e) => now.difference(e.spawnTime).inMilliseconds >= e.type.durationMs).toList();
    for (final e in expired) {
      _enemies.remove(e);
      _hp = (_hp - e.type.damage).clamp(0, 100);
      _combo = 0;
      SoundManager.miss(); _addPop('-${e.type.damage}霊力', Colors.redAccent, e.position);
  ...
  void _tapEnemy(Enemy enemy) {
    ...
    setState(() {
      ...
      if (enemy.currentHp <= 0) {
        ...
        _score += pts;
        SoundManager.tap(); _addPop('+$pts', mult > 1 ? Colors.amber : const Color(0xFFC8A8FF), enemy.position);
        _waveCount++;
  ...
  void _onStageClear() {
    _stageClear = true; SoundManager.stageClear();
    _spawnTimer?.cancel();
  ...
  void _useSkill(int i) {
    ...
    SoundManager.skill(); setState(() => _skillCDs[i] = skillDefs[i].cooldownSec);
  ...
  void _endGame({required bool win}) {
    if (!win) SoundManager.gameOver(); setState(() { _playing = false; _stageClear = false; });
    _spawnTimer?.cancel();
    _tickTimer?.cancel();
    _enemies.clear();
  }
      ...
      child: SafeArea(bottom: false, child: Row(
        ...
        children: [
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(stage.name, style: const TextStyle(fontSize: 9, color: Colors.white38, letterSpacing: 1.5)),
            const SizedBox(height: 3),
            Row(children: List.generate(stage.waves, (i) => Container(
  ...
  Widget _buildEnemy(Enemy enemy) {
    ...
    final progress = (1.0 - elapsed / enemy.type.durationMs).clamp(0.0, 1.0);
    return Positioned(
      ...
      child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: const Text('祓いを始める', style: TextStyle(color: Color(0xFFC8A8FF), fontSize: 15)),
          ),
        ),
      ])),
    );
